...
export async function generatePassport(
  ...
  async function loadLocalLogo() {
    try {
      // Try JPG first (your current logo format based on seal code)
      ...
      try {
        ...
        console.warn(
          "Logo not found in /public folder. Ensure file exists as vetsense_logo.jpg or vetsense_logo.png"
        );
        return null;
      }
    }
  ...
  async function fetchCircularSealUrl() {
    ...
    try {
      ...
      const apiUrl = base
        ? `${base}/api/seal/get-seal?type=CIRCULAR`
  ...
  doc.setFontSize(9);
  doc.text("07067677446 | Vetsense.equinecare@gmail.com", pageWidth / 2, 165, {
    align: "center",
  });
  ...
  const sections = [
    ...
    },
    {
      ...
      fields: [
        ...
        ["Last Vaccination Date", formatDate(latestMedical?.recordDate)],
