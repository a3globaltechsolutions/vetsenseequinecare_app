...
export async function generatePassport(
  ...
  // -------------------------
  // HORSE IMAGE PAGE (Page 2)
  // -------------------------
  console.log("[PDF Gen] Creating horse image page...");
  doc.addPage();
  ...
  doc.setFontSize(14);
  doc.setTextColor(0, 0, 0);
  doc.text("Horse Image", 20, 40);

  // Load and display horse image
  if (horseData.imageUrl) {
    try {
      const horseImageDataUrl = await fetchImageAsDataUrl(
        horseData.imageUrl,
        `horse-${horseData.id || "image"}`
      );

      if (horseImageDataUrl) {
        // Calculate dimensions to fit nicely on page
        const maxWidth = pageWidth - 40; // 20px margin on each side
        const maxHeight = 160; // Leave space for header and footer

        // Center the image
        const imgX = 20;
        const imgY = 50;

        const imgType = horseImageDataUrl.startsWith("data:image/jpeg")
          ? "JPEG"
          : "PNG";
        doc.addImage(
          horseImageDataUrl,
        ...
        // Add horse name below image
        doc.setFontSize(12);
        doc.setTextColor(0, 0, 0);
        doc.text(
          horseData.name || "Horse Image",
          pageWidth / 2,
          imgY + maxHeight + 15,
          { align: "center" }
        );
      ...
      console.warn("Failed to load horse image:", error.message);
      // Display placeholder if image fails
      doc.setFontSize(10);
      doc.setTextColor(128, 128, 128);
      doc.text("Horse image not available", pageWidth / 2, 120, {
        align: "center",
      });
    }
  } else {
    ...
    doc.setFontSize(10);
    doc.setTextColor(128, 128, 128);
    doc.text("No horse image uploaded", pageWidth / 2, 120, {
      align: "center",
    });
  }
  ...
  doc.text(`Page 2`, pageWidth / 2, pageHeight - 12, {
    align: "center",
  });
