...
// lib/pdf/passportGenerator.js - Production optimized version
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";
import QRCode from "qrcode";
import fs from "fs";
...
const MAX_IMAGE_WIDTH = 800; // Reduced from 1200 for faster processing
const JPEG_QUALITY = 60; // Reduced from 70 for smaller files
...
export async function generatePassport(
  ...
  console.log(`[PDF Gen] Complete in ${Date.now() - genStart}ms`);
  return doc;
}
